...
function canUpdate(id, typeName) {
  ...
  return (0, _snapshotPair.snapshotPair)(idPair).pipe((0, _operators.mergeMap)(pair => (0, _rxjs.combineLatest)([pair.draft.snapshots$, pair.published.snapshots$])), (0, _operators.switchMap)(_ref => {
    ...
    return type.liveEdit ? _.default.checkDocumentPermission('update', published || stub(idPair.publishedId, typeName)) : _.default.checkDocumentPermission('update', // note: we check against the published document (if it exist) with draft id since that's the
    // document that will be created as new draft when user edits it
    draft || _objectSpread(_objectSpread({}, published), {}, {
      _id: idPair.draftId
    }) || stub(idPair.draftId, typeName));
  }));
}
