...
function checkDeletePermission(snapshots) {
  var _snapshots$published;

  return (0, _rxjs.combineLatest)([_index.default.checkDocumentPermission('update', snapshots.draft), // NOTE: most reliable way to check whether the document "exists" is currently to see if it has a `_rev`
  // If the published document exists, the user *only* need access to delete (update) the draft
  (_snapshots$published = snapshots.published) !== null && _snapshots$published !== void 0 && _snapshots$published._rev ? _index.default.checkDocumentPermission('update', snapshots.published) : (0, _rxjs.of)({
    granted: true,
    reason: 'granted'
